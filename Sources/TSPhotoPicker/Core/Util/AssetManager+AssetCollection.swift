...
public extension AssetManager {
    ...
    /// - Returns: 相册列表
    static func fetchSmartAlbums(options: PHFetchOptions?) -> PHFetchResult<PHAssetCollection> {
    ...
    /// - Returns: 相册列表
    static func fetchUserAlbums(options: PHFetchOptions?) -> PHFetchResult<PHAssetCollection> {
        return PHAssetCollection.fetchAssetCollections(
            with: .album,
            subtype: .any,
            options: options
    ...
    static func enumerateAllAlbums(filterInvalid: Bool,
    ...
    static func createAssetCollection(for collectionName: String) -> PHAssetCollection? {
        ...
        assert(status == .denied || status == .restricted, "TSPhotoPicker Error: No permission")
        var assetCollection: PHAssetCollection?
