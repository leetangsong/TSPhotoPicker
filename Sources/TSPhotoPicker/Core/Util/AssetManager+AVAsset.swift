...
//  TSPhotoPicker
//
//  Created by leetangsong on 2022/11/25.
//
        ...
        return requestAVAsset(for: asset, version: version, deliveryMode: deliveryMode, isNetworkAccessAllowed: false, progressHandler: progressHandler) { result in
            DispatchQueue.main.async {
                ...
                case .success(let avResult):
                    if !avResult.avAsset.isPlayable {
                        self.requestAVAsset(for: asset,
                ...
                case .failure(let error):
                    switch error.error {
                    case .needSyncICloud:
                        let iCloudRequestID = self.requestAVAsset(for: asset,
                                                                  version: version,
