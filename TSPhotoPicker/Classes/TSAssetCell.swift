...
//        temp.isHidden = true
    ...
    lazy var btnSelect: UIButton  = {
        ...
        let image = TSPhotoPickerTool.getImage(with: "icon_selected")
        temp.setImage(image, for: .normal)
        temp.setImage(UIImage(), for: .selected)
        temp.handy.enlarged(with: UIEdgeInsets.init(top: 20, left: 0, bottom: 20, right: 20))
        temp.addTarget(self, action: #selector(btnSelectClick(send:)), for: .touchUpInside)
        let picker = ThemeImagePicker(keyPath: "") { _ in
            guard let color = TSPhotoPickerConfig.shared.themeColor.value() as? UIColor else{
                return UIImage()
            }
            return UIImage.handy.image(with: color)
        }
        
        temp.theme.setBackgroundImage(picker, forState: .selected)
    ...
    lazy var videoBottomView: UIImageView = {
        let temp = UIImageView()
        temp.image = TSPhotoPickerTool.getImage(with: "videoView")
        return temp
    }()
    lazy var videoImageView: UIImageView = {
        let temp = UIImageView()
        temp.image = TSPhotoPickerTool.getImage(with: "video")
    ...
    lazy var liveImageView: UIImageView = {
        let temp = UIImageView()
        temp.contentMode = .scaleAspectFill
        temp.image = TSPhotoPickerTool.getImage(with: "livePhoto")
...
//        }
    ...
    var model: TSPhotoModel!{
        ...
        didSet{
            let configure = TSPhotoPickerConfig.shared
            if model.type == .video{
            ...
            if let asset = model.asset{
                ...
                let requestID = TSPhotoManager.shared.getPhoto(with: asset, photoWidth: frame.width, completion: { [weak self] (photo, info, isDegraded) in
...
class TSAssetAddMoreCell: TSAssetCameraCell {
    private(set) lazy var tipLabel: UILabel = {
        ...
        label.textColor = UIColor.handy.color(rgba: "156,156,156")
        contentView.addSubview(label)
        return label
    }()
